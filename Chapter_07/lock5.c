...
int main()
{
	...
	printf("Process %d, tring F_RDLCK, region %d to %d\n", getpid(),
	...
	region_to_lock.l_start = 10;
	region_to_lock.l_len = 5;
	printf("Process %d, tring F_UNLCK, region %d to %d\n", getpid(),
			(int)region_to_lock.l_start, (int)(region_to_lock.l_start + 
				region_to_lock.l_len));
	res = fcntl(file_desc, F_SETLK, &region_to_lock);
	if (res == -1)
	...
	{
		printf("Process %d - obtained unlock region\n", getpid());
	}
	...
	region_to_lock.l_len = 5;
	printf("Process %d, tring F_WRLCK, region %d to %d\n", getpid(),
			(int)region_to_lock.l_start, (int)(region_to_lock.l_start + 
				region_to_lock.l_len));
	res = fcntl(file_desc, F_SETLK, &region_to_lock);
	if (res == -1)
	...
	region_to_lock.l_start = 16;
	region_to_lock.l_len = 5;
	printf("Process %d, tring F_WRLCK with wait, region %d to %d\n", getpid(),
			(int)region_to_lock.l_start, (int)(region_to_lock.l_start + 
				region_to_lock.l_len));
